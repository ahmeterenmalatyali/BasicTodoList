...
namespace TodoApp.Backend.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<IActionResult> Register([FromBody] RegisterDto dto)
        {
            ...
            // Varsayılan "Genel" kategorisini oluştur
            _context.Categories.Add(new Category { Name = "Genel", UserId = user.Id });
            await _context.SaveChangesAsync();

            var confirmationLink = $"http://localhost:3000/verify-email?token={verificationToken}";
            var emailBody = $"<h2>Hoş Geldin {user.Username}!</h2><p>Lütfen <a href='{confirmationLink}'>buraya tıklayarak</a> hesabını onayla.</p>";
